...
async def secretary_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👨‍💼 РЕЖИМ СЕКРЕТАРЯ FUNLANDIA\n\n"
        "Напишите свой вопрос или сообщение.\n"
        "Я передам его администратору FUNLANDIA.\n\n"
        "После ответа администратора сообщение придёт сюда.\n"
        "Для выхода: /stop"
        if lang == "ru" else
        "👨‍💼 FUNLANDIA KOTIB REJIMI\n\n"
        "Savolingiz yoki xabaringizni yozing.\n"
        "Men uni FUNLANDIA administratoriga yuboraman.\n\n"
...
async def secretary_client_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        if lang == "ru" else
        "✅ Xabaringiz administratorga yuborildi. Javobni kuting.",
        reply_markup=menu(lang)
    )
    return True
...
async def secretary_reply_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        "✍️ Напишите ответ клиенту одним сообщением."
    )
...
async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If the admin has selected "Reply to client", the next message
    # from the admin is sent to that client.
    if str(update.effective_user.id) == str(ADMIN_CHAT_ID):
        if await secretary_admin_reply(update, context):
            return

    # Client messages in secretary mode go to the admin.
...
def main():
    ...
    app.add_handler(CommandHandler("stop", secretary_stop))
